import reducer, { setSelectedImg, resetSelectedImg } from '../../../store/imageUiState/imageUiState'
...
describe('imageUiStateReducer', () => {
  ...
  it('should add image id as selectedImg if setSelected action is called', () => {
    ...
    expect(reducer(previousState, setSelectedImg({ id: imageMock1.id }))).toEqual({ selectedImg: imageMock1.id })
  })
  ...
  it('should reset selectedImg to empty string if resetSelectedImg action is called', () => {
    ...
    expect(reducer(previousState, resetSelectedImg())).toEqual({ selectedImg: '' })
  })
})